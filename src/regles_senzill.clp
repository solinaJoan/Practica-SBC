...
(defrule abstraccio-persona-gran
    ...
    (not (requisit-inferit (solicitant ?sol) (categoria-servei ServeiComercial)))
    =>
    ...
    (printout t "[ABSTRACCIO] " (instance-name ?sol) " necessita salut i comerços a prop" crlf)
)
...
(defrule presentacio-inici
    ...
    (assert (fase-completada (nom presentacio)))
    (printout t "================================================================" crlf)
    (printout t "                 Punts extres de les ofertes                    " crlf)
    (printout t "================================================================" crlf)
)
...
(defrule presentacio-punts-positius
    ...
    =>
    (printout t " Oferta: " (instance-name ?of) " per " (instance-name ?sol) " [+] " ?desc crlf)
)
...
;;;    =>
;;;    (printout t crlf "[DESCARTADA] " (instance-name ?of) " per " (instance-name ?sol) ": " ?motiu)
;;;)
