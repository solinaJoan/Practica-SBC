...
\begin{itemize}
    \item Molt Recomanable (≥70 punts)
    \item Adequat (≥40 punts)
...
\textbf{Motivació}: Tenir versions funcionals des del principi és molt més motivador que treballar durant setmanes en alguna cosa que no fa res fins al final. Cada prototip era un petit èxit que ens animava a continuar.
...
\begin{itemize}
    \item Retracting facts que ja no necessitem (com \texttt{dades-solicitant} un cop creat el sol·licitant)
    \item Evitant crear fets redundants amb condicions \texttt{(not (...))}
\end{itemize}

\subsubsection{Debugging de regles}

Quan una regla no es dispara quan esperem, pot ser difícil saber per què. CLIPS ofereix \texttt{(matches regla-name)} per veure quines condicions fallen, però hem trobat més útil afegir \texttt{debug-print} dins de les regles per veure quan s'executen.
...
\begin{itemize}
    ...
    \item \texttt{regles.clp}: Totes les regles del sistema, organitzades per fases
    \item \texttt{main.clp}: Funcions d'interacció i funció principal
    \item \texttt{presentacio.clp}: Regles específiques per a la presentació del Top 3
    \item \texttt{run.clp}: Script que carrega tot i executa el sistema
\end{itemize}
